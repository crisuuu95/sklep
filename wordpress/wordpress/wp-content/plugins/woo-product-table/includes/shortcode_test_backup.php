...
function wptf_shortcode_generator($atts = false) {
    ...
    if(isset( $atts['id'] ) && !empty( $atts['id'] ) && is_numeric( $atts['id'] ) ){
        ...
        $temp_number = $basics['temp_number'];
     ...
     */
    $wptf_product_short_order_by = ( isset($atts['sort_order_by']) && !empty($atts['sort_order_by']) ? $atts['sort_order_by'] : 'name' );  //4.0 default value is name: name//Removed date at V2.9 and Added menu_order as default
    $wptf_product_short = ( isset($atts['sort']) && !empty($atts['sort']) && in_array($atts['sort'], array('ASC', 'DESC')) ? $atts['sort'] : 'ASC' ); //Setting Default value ASC, Becuause added Randomize Option for Default at Bellow After Query
    ...
    $wptf_search_box = ( isset($atts['search_box']) && !empty($atts['search_box']) && $atts['search_box'] =='no' ? false : true ); //Set Search box Permission for now.
    ...
    $wptf_filter_box = ( isset($atts['filter_box']) && !empty($atts['filter_box']) && $atts['filter_box'] =='no' ? false : true ); //Set Search box Permission for now.
    ...
    */
    //Final Sku Start
    if($meta_value_sort && ( $wptf_product_short_order_by == 'meta_value' || $wptf_product_short_order_by == 'meta_value_num' ) ){
    ...
    //$html .= "<div data-add_to_cart='" . esc_attr( $wptf_add_to_cart_text ) . "'  data-adding_to_cart='" . esc_attr( $wptf_adding_to_cart_text ) . "' data-added_to_cart='" . esc_attr( $wptf_added_to_cart_text ) . "' data-out_of_stock_message='" . esc_attr( __( 'Out of Stock', 'wptf_pro' ) ) . "' data-select_all_items_message='" . esc_attr( __( 'Please select all items.', 'wptf_pro' ) ) . "' data-right_combination_message='" . esc_attr( __( 'Not available', 'wptf_pro' ) ) . "' data-add_to_cart='{$wptf_add_to_cart_text}' data-site_url='" . site_url() . "' id='table_id_" . $temp_number . "' class='wptf_temporary_wrapper_" . $temp_number . " wptf_product_table_wrapper " . $wptf_template . "_wrapper woocommerce'>"; //Table Wrapper Div start here with class. //Added woocommerce class at wrapper div in V1.0.4
            ...
            */
            ...
            if ( isset( $wptf_permitted_td['action'] ) ) {
               ...
               //var_dump($product->get_type());//external
                ...
                $wptf_add_to_cart_text_final = ( $product_type == 'grouped' || $product_type == 'external' || $wptf_add_to_cart_text == ' ' ? $product->add_to_cart_text() : $wptf_add_to_cart_text );//'?add-to-cart=' .  $data['id']; //home_url() . 
            ...
            //var_dump($wptf_table_column_keywords);
